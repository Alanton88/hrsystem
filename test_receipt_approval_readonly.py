...
def test_get_receipt_management_page():
    """測試取得驗收單作業頁面"""
    print("\n📋 測試取得驗收單作業頁面...")
    
    try:
        response = requests.get(f"{BASE_URL}/receipt-management")
        if response.status_code == 200:
            print("✅ 成功取得驗收單作業頁面")
            return True
        else:
...
def test_update_receipt_approval():
    ...
    # 測試資料
    test_data = {
        'purchase_no': '20250718-002',  # 使用一個存在的請購單號
        'approval_status': '核准',
        'approval_date': datetime.now().strftime('%Y%m%d'),
        'approver': '測試人員',
        'approval_note': '測試驗收簽核功能'
    }
...
def test_get_purchase_detail():
    ...
    try:
        response = requests.get(f"{BASE_URL}/purchase-detail/20250718-002")
        if response.status_code == 200:
            result = response.json()
            if result.get('success'):
                ...
                print(f"   驗收簽核人員: {data.get('驗收簽核人員', 'N/A')}")
                print(f"   驗收簽核日期: {data.get('驗收簽核日期', 'N/A')}")
...
def main():
    ...
    tests = [
        test_login,
        test_get_receipt_management_page,
